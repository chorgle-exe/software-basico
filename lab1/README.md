...
`mkdir inf1018` e `cd inf1018`.

## ex1.c
Programa que lê um valor float pela linha de comando (`argv[1]`), converte com `atof`,
soma 1 usando a função `foo` e imprime o resultado. `argc` conta quantos argumentos
foram digitados; `argv` é o vetor de strings com esses argumentos.
...
`labaux.c` guarda a implementação de `foo`; `ex2.c` guarda apenas o protótipo
`float foo(float pf);`. Cada `.c` é compilado separadamente em um `.o`
...
mostrando por que declarar protótipos é essencial em C.
...
- `c2` (char = '1'): 1 byte, valor armazenado `49` (código ASCII de `'1'`).
- `i` (int = 1): 4 bytes, mesmo valor lógico de `c1`, mas tipo maior.
- `v` (char[] = "1"): 2 bytes — o caractere `'1'` (49) + terminador `'\0'` (0).

**Conclusão geral:** o tipo da variável define quantos bytes ela ocupa; um mesmo
